...
server_port     = "#{ENV['PORT']}"      # get port for preview server from $PORT environment variable
server_ip       = "#{ENV['IP']}"        # IP for preview server from $IP environment variable
...
task :preview do
  ...
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll build --watch")
  compassPid = Process.spawn("compass watch")
  rackupPid = Process.spawn("rackup --host #{server_ip}  --port #{server_port}")
